...
#                        Documented in more detail in Stock Trade documentation.
...
# ==================================================================================================================
# Objective:
# This program is used to place stock order.
#
# ==================================================================================================================
# Logic:
...
def api_GetHistoricalPrices(Symb, Range, PeriodType, FrequencyType, Frequency, StartDate, EndDate):
    ...
    while (str_api_status == 'No OK'):
        ...
        if (content.status_code == 200):  # Process values if api-call successful
            ...
            func_display_info(190, 'Both', ['data: ' + '>>>' + str(data) + '<<<'])
            ...
            if (not data["empty"]):
                ...
                func_display_info(150, 'Both', ['Records retrieved from api_GetHistoricalPrices: ' + str(len(HistoricalPrices))])
                if (int_cntr > 10):
                    str_api_status = 'Ok'
                else:
                    func_display_info(0, 'Both', ['-' * 128])
    ...
    while (LastPrice ==  -1.23456789):
        ...
        try:
            ...
            if (content.status_code == 200):  # Process values if successful
                data = content.json()         # convert to python dictionary
                func_display_info(180, 'Both', ['data: ' + '>>>' + str(data) + '<<<'])

                for p in data.values(): LastPrice = float(p.get("lastPrice"))  # Get value from selected record

                func_display_info(180, 'Both', ['LastPrice: ' + '>>>' + str(LastPrice) + '<<<'])
...
def api_GetOrderByPath(OrderType, BuySell, str_account, obj_LineOrderStatus):  # OrderType is 'Single' or 'Conditional'
    global str_token_access, obj_ListLineOrderStatus
    func_check_token()

    url = r"https://api.tdameritrade.com/v1/accounts/{}/orders".format(str_account)
    ...
    data = content.json()  # convert to python dictionary
    func_display_info(190, 'Both', [data])
        ...
        if (OrderType == 'Single'):
            if (((obj_Order['status']) != 'CANCELED') and (obj_Order['orderType'] != 'MARKET')):
                if ((obj_Order['duration']  == 'GOOD_TILL_CANCEL') and
                    ...
                    (obj_Order['accountId'] == int(str_account)) and
    ...
    if (OrderType == 'Single'):
        ...
        func_display_info(140, 'Both', ['Possible Orders found not being used: ' + str(ListOrderSubmitted)])
    ...
    if (OrderType == 'Conditional'):
        ...
        func_display_info(140, 'Both', ['Possible Orders not being used: Buy ' + str(ListOrderSubmittedBuy) + ' Sell '+ str(ListOrderSubmittedSell)])
...
def api_GetOrderStatus(BuySell, str_account, obj_LineOrderStatus):
    global str_token_access, obj_ListLineOrderStatus
    global int_max_retries
    ...
    while (OrderStatus == 'No OK'):
        ...
        url = r"https://api.tdameritrade.com/v1/accounts/{}/orders/{}".format(str_account, OrderNumber)
        params = {}
        headers = {"HTTP_HOST": "http://localhost", "Authorization": "Bearer " + str_token_access}
    ...
    if (BuySell == 'BUY'):
        if ((data['duration'] == 'GOOD_TILL_CANCEL') and
            ...
            (data['accountId'] == int(str_account))):
            for data_orderLegCollection in data["orderLegCollection"]:
...
def api_GetTokenAuthorization(TokenType):
    ...
    data = content.json()  # convert to python dictionary
    func_display_info(150, 'Both', [str(data)])

    if (TokenType == 'TokenAccess'):
        func_display_info(150, 'Both', ['Token Access: ' + data['access_token']])
    if (TokenType == 'TokenRefresh'):
        func_display_info(150, 'Both', ['Token Access: ' + data['access_token']])
        func_display_info(150, 'Both', ['Token Refresh: ' + data['refresh_token']])
...
def api_PlaceOrder(OrderType, BuySell, str_account, obj_LineOrderStatus):
    # OrderType is 'Single' or 'Conditional'
    global str_token_access
    global lst_stock_regularMarketOnly_OTC  # list of stocks with restrictions to place single orders
    func_check_token()

    if (obj_LineOrderStatus.symbol in lst_stock_regularMarketOnly_OTC):
...
class cls_LineMarketIndicators:
    ...
    def load_from_file(self, DateTimeNow_UnixEpoch_TDAFormat):
        if (self.need_load_from_file == 'Yes'):
            ...
            if (os.path.isfile('Stock_' + self.symbol.strip() + '.txt')):                                # History already exists
                ...
                self.last_update = DateTimeNow_UnixEpoch_TDAFormat - (3 * 365 * 24 * 60 * 60 * 1000)
        func_display_info(160, 'Both', ['Historical records loaded from file: ' + str(len(self.list_prices))])
    ...
    def load_from_online(self, DateTimeNow_UnixEpoch_TDAFormat):
        ...
        func_display_info(150, 'Both', ['Last Update: ' + str(self.last_update)])
        ...
        if ((self.last_update < (DateTimeNow_UnixEpoch_TDAFormat - (5 * 24 * 60 * 60 * 1000))) or     # If Last Update older than 5 days, get 1 min Prices for the last 5 days
            ...
            func_display_info(180, 'Both', ['Getting historical prices because last update is greater than 5 minutes mark'])
        ...
        if (self.symbol in lst_stock_regularMarketOnly_OTC):
            if (bool_regularMarket):  # Check if current NY time is regularMarket
                ...
                func_display_info(180, 'Both', ['DateTimeNow_UnixEpoch_TDAFormat: ' + str(DateTimeNow_UnixEpoch_TDAFormat)])
        else:
        ...
        func_display_info(180, 'Both', ['Total of Prices loaded from online: ' + str(len(self.list_prices))])
        ...
        func_display_info(180, 'Both', ['Total of Prices after deleting price zero: ' + str(len(self.list_prices))])

        # Sort records; oldest records first
        self.list_prices = sorted(self.list_prices, reverse = True)
        func_display_info(180, 'Both', ['Total of Records after sorting: ' + str(len(self.list_prices))])
        ...
        func_display_info(180, 'Both', ['Total of Records after deleting duplicates: ' + str(len(self.list_prices))])
        ...
        for objPrice in self.list_prices:
            ...
            func_display_info(180, 'Both', ['Symb: ' + self.symbol + ' Date: ' + str(objPrice[0]) + ' Format: ' + time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(round(objPrice[0] / 1000, 0))) + ' Price: ' + '{:.8f}'.format(round(objPrice[1], 2)) + ' Range: ' + str(objPrice[2]) + ' ---(value)'])
            ...
            if (objPricePPPP[2] == objPrice[2]):
                if ((objPricePPP[2] != objPrice[2]) and (objPricePP[2] != objPrice[2]) and  (objPriceP[2] != objPrice[2])):  # 3 out of sequence
                    if (objPriceP[2] > 1):
                        ...
                        func_display_info(180, 'Both', ['Symb: ' + self.symbol + ' Date: ' + str(objPriceP[0]) + ' Format: ' + time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(round(objPriceP[0] / 1000, 0))) + ' Price: ' + '{:.8f}'.format(round(objPriceP[1], 2)) + ' Range: ' + str(objPriceP[2]) + ' -- (remove P)'])
                        objPriceP = objPrice
                    if (objPricePP[2] > 1):
                        self.list_prices.remove(objPricePP)
                        func_display_info(180, 'Both', ['Symb: ' + self.symbol + ' Date: ' + str(objPricePP[0]) + ' Format: ' + time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(round(objPricePP[0] / 1000, 0))) + ' Price: ' + '{:.8f}'.format(round(objPricePP[1], 2)) + ' Range: ' + str(objPricePP[2]) + ' -- (remove PP)'])
                        objPricePP = objPrice
                    if (objPricePPP[2] > 1):
                        self.list_prices.remove(objPricePPP)
                        func_display_info(180, 'Both', ['Symb: ' + self.symbol + ' Date: ' + str(objPricePPP[0]) + ' Format: ' + time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(round(objPricePPP[0] / 1000, 0))) + ' Price: ' + '{:.8f}'.format(round(objPricePPP[1], 2)) + ' Range: ' + str(objPricePPP[2]) + ' -- (remove PPP)'])
                        objPricePPP = objPrice
        ...
        func_display_info(180, 'Both', ['Total of Records after deleting records with mix ranges, first pass: ' + str(len(self.list_prices))])
        ...
        func_display_info(180, 'Both', ['Total of Records after deleting records with mix ranges, second pass: ' + str(len(self.list_prices))])

        # Sort records again; oldest records first
        self.list_prices = sorted(self.list_prices, reverse = True)
        func_display_info(180, 'Both', ['Total of Records after sorting: ' + str(len(self.list_prices))])

        func_display_info(150, 'Both', ['Total of Records after sorting: ' + str(self.symbol) + ' ' + str(len(self.list_prices))])
    ...
    def calc_rsi(self, Period, DateMark):
        ...
        func_display_info(170, 'Both', ['Period: ' + Period])
        ListValues = []
            ...
            if ((Period == '4hr') or (Period == '1hr') or (Period == '30min') or (Period == '15min')):
                if ((objPrice[0] <= DateMark) and (len(ListValues) > -1)):
                    ...
                    if ((Period == '15min') and (len(ListValues) < 40)):
                       func_display_info(180, 'Both', ['Selected - Symb: ' + self.symbol + ' Period: ' + Period + ' Date: ' + str(objPrice[0]) + ' Format: ' + time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(round(objPrice[0] / 1000, 0))) + ' Price: ' + '{:.8f}'.format(round(objPrice[1], 2)) + ' Range: ' + str(objPrice[2])])

                    MovingDate = objPrice[0]  # it could be DateMark or objPrice[0]. Keeping objPrice[0] to compensate with non-tradeing after hours
                    func_display_info(182, 'Both', ['MovingDate: ' + str(MovingDate)])
                    ...
                    func_display_info(182, 'Both', ['DateAt6AM_Basic: ' + str(DateAt6AM_Basic)])

                    DateAt6AM_UnixEpoch = time.mktime(DateAt6AM_Basic.timetuple())
                    func_display_info(182, 'Both', ['DateAt6AM_UnixEpoch: ' + str(DateAt6AM_UnixEpoch)])

                    DateAt6AM_UnixEpoch_TDAFormat = DateAt6AM_UnixEpoch * 1000
                    func_display_info(182, 'Both', ['DateAt6AM_UnixEpoch_TDAFormat: ' + str(DateAt6AM_UnixEpoch_TDAFormat)])

                    # Calcualte 7 PM Date, prior day
                    DateAt7PM_UnixEpoch_TDAFormat = DateAt6AM_UnixEpoch_TDAFormat - (11 * 60 * 60 * 1000)  # 6AM -> 7PM, 11 hours
                    func_display_info(182, 'Both', ['DateAt7PM_UnixEpoch_TDAFormat: ' + str(DateAt7PM_UnixEpoch_TDAFormat)])
                    ...
                    if ((MovingDate - DateAt6AM_UnixEpoch_TDAFormat) < TimeDelta):  # No space for delta; need to compensate for dead zone
                        ...
                        func_display_info(182, 'Both', ['AdjustTime_TDAFormat: ' + str(AdjustTime_TDAFormat)])
                    ...
                    if ((Period == '15min') and (len(ListValues) < 40)):
                       func_display_info(180, 'Both', ['Selected - Symb: ' + self.symbol + ' Period: ' + Period + ' Date: ' + str(objPrice[0]) + ' Format: ' + time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(round(objPrice[0] / 1000, 0))) + ' Price: ' + '{:.8f}'.format(round(objPrice[1], 2)) + ' Range: ' + str(objPrice[2]) + ' ---(skip)'])

        func_display_info(170, 'Both', ['len(ListValues): ' + str(len(ListValues))])
    ...
    def save(self):
        ...
        if os.path.exists('Stock_' + self.symbol.strip() + '.txt'):
            os.remove('Stock_' + self.symbol.strip() + '.txt')
            time.sleep(float_time_delay_io)  # Delay given to remove/delete file
        outF = open('Stock_' + self.symbol.strip() + '.txt', "a")
        for objPrice in self.list_prices:
...
class cls_LineOrderStatus:
    def __init__(self, str_from_file, str_line):   # attributes
        if str_from_file == "FromFile":
            ...
            self.seq                    = int(str_line[34:37].strip())
            self.trigger_buy_gap        = int(str_line[38:41].strip())
            self.trigger_buy_rsi_wk     = int(str_line[42:45].strip())
            self.trigger_buy_rsi_day    = int(str_line[46:49].strip())
            self.trigger_buy_rsi_4hr    = int(str_line[50:53].strip())
            self.trigger_buy_rsi_1hr    = int(str_line[54:57].strip())
            self.trigger_buy_rsi_30m    = int(str_line[58:61].strip())
            self.trigger_buy_rsi_15m    = int(str_line[62:65].strip())
            self.trigger_buy_adj_price  = float(str_line[66:71].strip())
            self.trigger_sell_rsi_wk    = int(str_line[72:75].strip())
            self.trigger_sell_rsi_day   = int(str_line[76:79].strip())
            self.trigger_sell_rsi_4hr   = int(str_line[80:83].strip())
            self.trigger_sell_rsi_1hr   = int(str_line[84:87].strip())
            self.trigger_sell_rsi_30m   = int(str_line[88:91].strip())
            self.trigger_sell_rsi_15m   = int(str_line[92:95].strip())
            self.trigger_sell_adj_price = float(str_line[96:101].strip())
            self.trigger_buy_shares_amt = float(str_line[102:108].strip())

            self.order_buy_status       = str_line[161:171].strip()
            if (self.order_buy_status != ""):
                self.order_buy_rsi_wk   = int(str_line[109:112].strip())
                self.order_buy_rsi_day  = int(str_line[113:116].strip())
                self.order_buy_rsi_4hr  = int(str_line[117:120].strip())
                self.order_buy_rsi_1hr  = int(str_line[121:124].strip())
                self.order_buy_rsi_30m  = int(str_line[125:128].strip())
                self.order_buy_rsi_15m  = int(str_line[129:132].strip())
                self.order_buy_number   = int(str_line[133:143].strip())
                self.order_buy_shares   = int(str_line[144:150].strip())
                self.order_buy_price    = float(str_line[151:160].strip())
                self.order_buy_status   = str_line[161:171].strip()
            else:
            ...
            self.order_sell_status      = str_line[224:234].strip()
            if (self.order_sell_status != ""):
                self.order_sell_rsi_wk  = int(str_line[172:175].strip())
                self.order_sell_rsi_day = int(str_line[176:179].strip())
                self.order_sell_rsi_4hr = int(str_line[180:183].strip())
                self.order_sell_rsi_1hr = int(str_line[184:187].strip())
                self.order_sell_rsi_30m = int(str_line[188:191].strip())
                self.order_sell_rsi_15m = int(str_line[192:195].strip())
                self.order_sell_number  = int(str_line[196:206].strip())
                self.order_sell_shares  = int(str_line[207:213].strip())
                self.order_sell_price   = float(str_line[214:223].strip())
                self.order_sell_status  = str_line[224:234].strip()
            else:
    ...
    def place_order(self, float_prior_order_buy_price, obj_LineMarketIndicators, str_account):
        global bool_preMarket, bool_regularMarket, bool_postMarket
        global lst_stock_regularMarketOnly_OTC
        func_check_market_hours()
        if (self.order_buy_status.strip() == '' ):  # Check if Buy Order was placed already
            if (os.path.isfile("PlaceBuyOrders.txt")):  # Check if file PlaceBuyOrders exits
                if (((self.symbol     in lst_stock_regularMarketOnly_OTC) and ((self.type == 'Single') and (bool_regularMarket))) or                                       # Check if stock     in regularMarket and single order during regularMarket
                    ((self.symbol not in lst_stock_regularMarketOnly_OTC) and ((self.type == 'Single') and (bool_preMarket or bool_regularMarket or bool_postMarket))) or  # Check if stock not in regularMarket and single order during preMarket, regularMarket and postMarket
                    ...
                    if ((self.trigger_buy_rsi_wk  > obj_LineMarketIndicators.rsi_wk ) and  # Check for triggers to place Buy Order
                        ...
                        if (self.type == 'Single'):       # If Single Order
                            self.order_buy_status = api_PlaceOrder('Single', 'BUY', str_account, self)                    # default status Transition
                            self.order_buy_number = api_GetOrderByPath('Single', 'BUY', str_account, self)                # Buy Order Number received
                            self.order_buy_status = api_GetOrderStatus('BUY', str_account, self)                          # actual status
                        ...
                        if (self.type == 'Conditional'):  # If Conditional Order
                            ...
                            self.order_buy_status = api_PlaceOrder('Conditional', 'BUY', str_account, self)         # default status Transition
                            OrderNumbers = api_GetOrderByPath('Conditional', 'BUY', str_account, self)              # Buy and Sell Order Numbers received
        ...
        if ((self.order_buy_status.strip() == 'FILLED') and (self.order_sell_status.strip() == '')):        # Check if Sell Order is already in place; Conditional Order not apply
            if ((self.type == 'Single') and (bool_preMarket or bool_regularMarket or bool_postMarket)):     # Check if single order during preMarket, regularMarket and postMarket
                if ((self.trigger_sell_rsi_wk  < obj_LineMarketIndicators.rsi_wk ) and                      # Check for triggers to place Sell Order
                    ...
                    self.order_sell_status = api_PlaceOrder('Single', 'SELL', str_account, self)                   # default status Transition
                    self.order_sell_number = api_GetOrderByPath('Single', 'SELL', str_account, self)
                    self.order_sell_status = api_GetOrderStatus('SELL', str_account, self)                         # actual status
                    obj_ListLineOrderStatus.save()                                                                 # Save Order Status file as contingency
    ...
    def print(self):
        ...
        str_line = str_line + ("   "    + str(self.trigger_buy_gap                       ))[-3:] + " "
        str_line = str_line + ("   "    + str(self.trigger_buy_rsi_wk                     ))[-3:] + " "
        str_line = str_line + ("   "    + str(self.trigger_buy_rsi_day                    ))[-3:] + " "
        str_line = str_line + ("   "    + str(self.trigger_buy_rsi_4hr                    ))[-3:] + " "
        str_line = str_line + ("   "    + str(self.trigger_buy_rsi_1hr                    ))[-3:] + " "
    ...
    def reset_order(self):
        if ((self.order_buy_status == 'FILLED') and (self.order_sell_status == 'FILLED')):                      # Initialize order cycle.
            func_display_info(130, 'Both', [ 'Reset: ' + str(self.print())])
    ...
    def update_order_status(self, str_account):
        if ((self.order_buy_status.strip() != '') and (self.order_buy_status != 'FILLED')):                     # Buy Order in Transition
            self.order_buy_status = api_GetOrderStatus('BUY', str_account, self)
        if ((self.order_buy_status == 'FILLED') and (self.order_sell_status.strip() != '') and (self.order_sell_status != 'FILLED')):    # Sell Order in Transition
            self.order_sell_status = api_GetOrderStatus('SELL', str_account, self)
...
class cls_ListLineBuySellStatus:
    ...
    def initial_load(self, obj_ListLineOrderStatus):
        ...
        func_display_info(180, 'Both', ['BuySell monitored: '])
        for obj_LineBuySellStatus in self.List:
            func_display_info(180, 'Both', [str(obj_LineBuySellStatus.symbol) + ' ' + str(obj_LineBuySellStatus.period) + ' ' + str(obj_LineBuySellStatus.repetitions)])
...
class cls_ListLineMarketIndicators:
    ...
    def initial_load(self, obj_ListLineOrderStatus):  # load symbols
        ...
        func_display_info(180, 'Both', ['Stocks monitored: '])
        for obj_LineMarketIndicators in obj_ListLineMarketIndicators.List:
            func_display_info(180, 'Both', [str(obj_LineMarketIndicators.symbol)])
...
class cls_ListLineOrderStatus:
    ...
    def load(self):
        with open("OrderStatus.txt", "r") as fileOrderStatus:
        ...
        func_display_info(150, 'Both', ['List of Order Status Loaded:'])
        for obj_LineOrderStatus in self.List:
            func_display_info(150, 'Both', [obj_LineOrderStatus.print()])

        self.List = sorted(self.List, key=attrgetter("symbol", "period", "type", "seq"), reverse=False)

        func_display_info(150, 'Both', ['List of Order Status Sorted:'])
        for obj_LineOrderStatus in self.List:
            func_display_info(150, 'Both', [obj_LineOrderStatus.print()])

        func_display_info(150, 'Both', ['Validate Duplicate Records in List of Order Status'])
        ...
        func_display_info(150, 'Both', ['Validate Skipped-Sequence Records in List of Order Status'])
    ...
    def save(self):
        copyfile("OrderStatusHeader.txt", "OrderStatus.txt")
        time.sleep(float_time_delay_io)
        outF = open("OrderStatus.txt", "a")
        for obj_LineOrderStatus in self.List:
...
#   if (len(ListValues) < 15):  #  modified interation limit to allow new stock IPOs, e.g. COIN
    ...
    func_display_info(150, 'Both', ['Total Records: ' + str(len(ListValues))])
    ...
    func_display_info(180, 'Both', ['RSI: ' + str(ListRSI[len(ListValues) - 1])])
...
def func_check_token():
    ...
    str_token_access_datetime_request = read_config.get("Access", "str_token_access_datetime_request")
    dt_token_access_datetime_request = datetime.strptime(str_token_access_datetime_request, "%Y%m%d %H:%M:%S")
    func_display_info(150, 'Both', ['str_token_access_datetime_request >>>' + str_token_access_datetime_request + '<<<'])

    str_token_refresh_datetime_request = read_config.get("Access", "str_token_refresh_datetime_request")
    dt_token_refresh_datetime_request = datetime.strptime(str_token_refresh_datetime_request, "%Y%m%d %H:%M:%S")
    func_display_info(150, 'Both', ['str_token_refresh_datetime_request >>>' + str_token_refresh_datetime_request + '<<<'])

    str_token_access = read_config.get("Access", "str_token_access")
    func_display_info(150, 'Both', ['str_token_access >>>' + str_token_access + '<<<'])

    str_token_refresh = read_config.get("Access", "str_token_refresh")
    func_display_info(150, 'Both', ['str_token_refresh >>>' + str_token_refresh + '<<<'])
    ...
    if ((dt_token_access_datetime_request + timedelta(minutes=int_token_access_time_limit)) < (datetime.now() + timedelta(minutes=2))):
        ...
        func_display_info(150, 'Both', ['New str_token_access >>>' + str_token_access + '<<<'])
        ...
        read_config.set("Access", "str_token_access_datetime_request", str_token_access_datetime_request)
        read_config.set("Access", "str_token_access", str_token_access)
        with open('Trade_Config.ini', 'w') as configfile:
    ...
    if ((dt_token_refresh_datetime_request + timedelta(minutes=int_token_refresh_time_limit)) < (datetime.now() + timedelta(days=10))):
        ...
        func_display_info(150, 'Both', ['New str_token_refresh >>>' + str_token_refresh + '<<<'])
        ...
        read_config.set("Access", "str_token_refresh_datetime_request", str_token_refresh_datetime_request)
        read_config.set("Access", "str_token_refresh", str_token_refresh)
        with open('Trade_Config.ini', 'w') as configfile:
            read_config.write(configfile)
...
if __name__ == "__main__":
    ...
    read_config.read("Trade_Config.ini")

    global int_debug  # 0 no events, 1 high level events, 50 medium level events, 100 small events (all)
    str_debug = read_config.get("My App", "str_debug")
    int_debug = int(str_debug)
    func_display_info(150, 'Both', ['str_debug >>>' + str_debug + '<<<'])

    global float_time_delay_io
    str_time_delay_io = read_config.get("My App", "str_time_delay_io")
    float_time_delay_io = float(str_time_delay_io)
    func_display_info(150, 'Both', ['str_time_delay_io >>>' + str_time_delay_io + '<<<'])

    global float_time_delay_process
    str_time_delay_process = read_config.get("My App", "str_time_delay_process")
    float_time_delay_process = float(str_time_delay_process)
    func_display_info(150, 'Both', ['str_time_delay_process >>>' + str_time_delay_process + '<<<'])

    global int_max_retries
    str_max_retries = read_config.get("My App", "str_max_retries")
    int_max_retries = int(str_max_retries)
    func_display_info(150, 'Both', ['str_max_retries >>>' + str_max_retries + '<<<'])

    global int_token_access_time_limit, str_token_access
    str_token_access_time_limit = read_config.get("My App", "str_token_access_time_limit")
    int_token_access_time_limit = int(str_token_access_time_limit)
    func_display_info(150, 'Both', ['str_token_access_time_limit >>>' + str_token_access_time_limit + '<<<'])

    global int_token_refresh_time_limit, str_token_refresh
    str_token_refresh_time_limit = read_config.get("My App", "str_token_refresh_time_limit")
    int_token_refresh_time_limit = int(str_token_refresh_time_limit)
    func_display_info(150, 'Both', ['str_token_refresh_time_limit >>>' + str_token_refresh_time_limit + '<<<'])

    global str_consumer_key
    str_consumer_key = read_config.get("My App", "str_consumer_key")
    func_display_info(150, 'Both', ['str_consumer_key >>>' + str_consumer_key + '<<<'])

    str_user_id = read_config.get("TD Ameritrade", "str_user_id")
    func_display_info(150, 'Both', ['str_user_id >>>' + str_user_id + '<<<'])

    str_account = read_config.get("TD Ameritrade", "str_account")
    func_display_info(150, 'Both', ['str_account >>>' + str_account + '<<<'])

    func_display_info(150, 'Both', ['Verify Necessary Files Exist. . .'])
    if not (os.path.isfile("Trade_Config.ini")):
        func_display_info(0, 'Both', ['-' * 128])
        func_display_info(0, 'Both', ['* * * ERROR * * * Missing file Trade_Config.ini!'])
        func_display_info(-1, 'Both', ['-' * 128])
    if not (os.path.isfile("Log.txt")):
        func_display_info(0, 'Both', ['-' * 128])
        func_display_info(0, 'Both', ['* * * ERROR * * * Missing file Log.txt!'])
        func_display_info(-1, 'Both', ['-' * 128])
    if not (os.path.isfile("OrderStatus.txt")):
        func_display_info(0, 'Both', ['-' * 128])
        func_display_info(0, 'Both', ['* * * ERROR * * * Missing file OrderStatus.txt!'])
        func_display_info(-1, 'Both', ['-' * 128])
    if not (os.path.isfile("OrderStatusHeader.txt")):
        func_display_info(0, 'Both', ['-' * 128])
    ...
    global lst_stock_regularMarketOnly_OTC  # list of stocks with restrictions to place single limit orders (only allowed during regularMarket)
    lst_stock_regularMarketOnly_OTC = ['VWDRY']

    while not (os.path.isfile('Trade_Exit.txt') or (dt_trading_timestamp > dt_trading_timestamp.replace(hour=23, minute=50, second=0, microsecond=0))):  # Main loop * * * Begin of Loop * * *  # If file Exit exists or near to midnight, then Exit loop.

        dt_trading_timestamp = datetime.today() + timedelta(minutes=60)
        func_display_info(20, 'Both', ['dt_trading_timestamp: ' + str(dt_trading_timestamp) + ' str_account: ' + str_account + ' str_user_id: ' + str_user_id + ' str_time_delay_process: ' + str_time_delay_process])
        ...
        for obj_LineOrderStatus in obj_ListLineOrderStatus.List:
            func_display_info(140, 'Both', ['Processing Order for: ' + obj_LineOrderStatus.symbol + ' ' + obj_LineOrderStatus.type + ' ' + obj_LineOrderStatus.period + ' ' + str(obj_LineOrderStatus.seq)])
            for obj_LineMarketIndicators in obj_ListLineMarketIndicators.List:
                if  (obj_LineOrderStatus.symbol == obj_LineMarketIndicators.symbol):
                    if (obj_LineOrderStatus.seq == 1):
                    ...
                    func_display_info(150, 'Both', ['Symbol - Prior Order Buy Price: ' + obj_LineOrderStatus.symbol + ' ' + obj_LineOrderStatus.period + ' ' + obj_LineOrderStatus.type + ' ' + str(obj_LineOrderStatus.seq) + ' ' + str(float_prior_order_buy_price)])
                    obj_LineOrderStatus.place_order(float_prior_order_buy_price, obj_LineMarketIndicators, str_account)
                    obj_LineOrderStatus.update_order_status(str_account)
                    obj_LineOrderStatus.reset_order()

    obj_ListLineOrderStatus.save()  # Save Order Status file before exit
    obj_ListLineMarketIndicators.save()
    if (os.path.isfile('Trade_Exit.txt')):
        os.rename('Trade_Exit.txt', 'Trade_ExitNO.txt')  # Ready to start the process again
