...
# ==================================================================================================================
# Objective:
# This program is used to schedule programs at specific times.
#
# ==================================================================================================================
# Logic:
...
def func_run_process(int_time_hours, int_time_minutes, int_runtime_minutes):
    ...
    if (os.path.isfile('Scheduler_Exit.txt')):
        return
    ...
    if (os.path.isfile('Trade_Exit.txt')):
        os.rename('Trade_Exit.txt', 'Trade_ExitNO.txt')
    ...
    while (int_counter < int_runtime_minutes):
        if (os.path.isfile('Scheduler_Exit.txt')):
            if (os.path.isfile('Trade_ExitNO.txt')):
                os.rename('Trade_ExitNO.txt', 'Trade_Exit.txt')
            return
...
if __name__ == "__main__":
    ...
    if not (os.path.isfile('Scheduler_Exit.txt')):
        print("delay 15 minutes for scheduler process to finish.")
        time.sleep(15 * 60)  # delay time
    print('finish scheduler cycle.')

    if (os.path.isfile('Scheduler_Exit.txt')):
        os.rename('Scheduler_Exit.txt', 'Scheduler_ExitNO.txt')  # Ready to start the process again
    print('The End.')
